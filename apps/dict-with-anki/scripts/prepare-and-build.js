...
async function main() {
    ...
    const originalReaderPath = path.join(
        appRoot,
        "node_modules",
        "@dict",
        "reader",
    ...
    try {
        ...
        execSync(
            `pnpm --filter @dict/reader pack --pack-destination ${tempPackDir}`,
            { stdio: "inherit" },
        );
        ...
        execSync(
            "pnpm -F @dict/app exec electron-vite build && pnpm -F @dict/app exec electron-builder",
            { stdio: "inherit" },
        );
    } catch (error) {
        ...
        if (fs.existsSync(backupPath)) {
            ...
            console.log(
                'Backup not found, running "pnpm install" to restore dependencies...',
            );
